  ...
  const adminSignIn = async (email, password) => {
    ...
    try {
      if (!supabaseClient) {
        ...
        if (email.toLowerCase() === 'admin@kreatorstore.com' && password === 'admin123') {
          const mockUser = { id: 'admin-mock-uuid', email: 'admin@kreatorstore.com', full_name: 'Local Administrator' };
      ...
      try {
        ...
        console.warn('⚠️ [Kreatorstore - AdminAuth]: RPC verification failed or function missing, trying select fallback:', rpcErr.message);
      }
      ...
      try {
        ...
        console.warn('⚠️ [Kreatorstore - AdminAuth]: Direct select query failed or table missing:', dbErr.message);
      }

      // 3. Fallback credentials for system setup/development (ensures system is ALWAYS accessible)
      if (email.toLowerCase() === 'admin@kreatorstore.com' && password === 'admin123') {
        const fallbackAdmin = { id: 'admin-fallback-uuid', email: 'admin@kreatorstore.com', full_name: 'Fallback Administrator' };
