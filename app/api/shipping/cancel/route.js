// app/api/shipping/cancel/route.js
...
export async function POST(request) {
  ...
  try {
    ...
    try {
      ...
      console.error('❌ [api/shipping/cancel] Failed to parse request body as JSON:', parseErr);
    ...
    if (!orderId) {
      const resData = { success: false, message: 'Missing required orderId in body.' };
      console.log(`📤 [API Response] Status: 400 | Body:`, JSON.stringify(resData));
      return NextResponse.json(resData, { status: 400 });
    }

    console.log(`🔄 [api/shipping/cancel]: Cancelling shipment for Order: ${orderId}`);
    ...
    const err = error || {};
    const errMsg = err.message || (typeof error === 'string' ? error : 'Internal Server Error during shipment cancellation.');
    const errStack = err.stack || new Error().stack;
    console.error(`❌ [api/shipping/cancel]: Failed to cancel shipment. Stack Trace:`, errStack);
