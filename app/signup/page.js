...
export default function SignupPage() {
  ...
  const handleSignup = async (e) => {
    ...
    startLoading();
    try {
      console.log('🔄 [Kreatorstore - MerchantSignup]: Calling signUp service for email:', email);
      
      const { data, error } = await authService.signUp(email, password, storeName);
      if (error) throw error;

      console.log('✅ [Kreatorstore - MerchantSignup]: Signup successful, verification pending.', data);
      setIsSignedUp(true);
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      console.log(`🔄 [Kreatorstore - MerchantSignup]: Triggering OAuth signup for ${provider} with callback ${callbackUrl}`);
      ...
      console.error(`❌ [Kreatorstore - MerchantSignup]: ${provider} OAuth error:`, err);
            ...
            <form onSubmit={handleSignup} className="login-form">
              <div className="form-group">
                ...
                <input
                  ...
                  required
                />
                <span className="field-hint">Your store URL will be: kreatorstore.com/store/{storeName.toLowerCase().replace(/\s+/g, '-')}</span>
              </div>
