...
const compressImage = (file, maxWidth = 800, maxHeight = 600) => {
  return new Promise((resolve) => {
    const timeout = setTimeout(() => {
      console.warn('[Kreatorstore - Image Compression] Timeout reached. Returning empty string.');
      resolve('');
    }, 4000);
...
const runWithTimeoutAndRetry = async (queryFn, maxRetries = 3, timeoutMs = 30000) => {
  ...
  while (true) {
    ...
    try {
      ...
      console.warn(`⚠️ [Kreatorstore - Query] Attempt ${attempt} failed:`, err.message || err);
      ...
      if (attempt < maxRetries) {
        console.log(`🔄 [Kreatorstore - Query] Retrying query in 1.5s (Attempt ${attempt + 1}/${maxRetries})...`);
...
export const storeService = {
  ...
  getStoreBySlug: async (slug) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`[Kreatorstore] Store with slug "${slug}" not found in Supabase.`);
        return null;
      }
  ...
  uploadLogo: async (file, storeId) => {
    ...
    while (attempt < maxAttempts) {
      attempt++;
      try {
        console.log(`[Kreatorstore - Storage] Uploading logo (Attempt ${attempt}/${maxAttempts})...`);
        ...
        if (error) {
          console.warn(`[Kreatorstore - Storage] Logo upload attempt ${attempt} error:`, error.message || error);
          ...
          if (isBucketErr) {
            console.warn('[Kreatorstore - Storage] Storage bucket error/not found. Falling back to compressed Base64 data URL immediately.');
        ...
        console.warn(`[Kreatorstore - Storage] Exception on logo upload attempt ${attempt}:`, err.message || err);
        ...
        if (isBucketErr || isTimeout) {
          console.warn('[Kreatorstore - Storage] Storage bucket exception/timeout. Falling back to compressed Base64 data URL immediately.');
          const base64Url = await compressImage(file, 200, 200);
          console.log("Upload Success:", base64Url);
          return base64Url;
        }
        ...
        console.warn('[Kreatorstore - Storage] Falling back to compressed Base64 data URL for logo.');
        const base64Url = await compressImage(file, 200, 200);
        console.log("Upload Success:", base64Url);
        return base64Url;
      }
  ...
  uploadBanner: async (file, storeId) => {
    ...
    while (attempt < maxAttempts) {
      attempt++;
      try {
        console.log(`[Kreatorstore - Storage] Uploading banner (Attempt ${attempt}/${maxAttempts})...`);
        const uploadPromise = supabaseClient.storage
          .from('store-assets')
          .upload(filePath, file, { upsert: true });
        ...
        if (error) {
          console.warn(`[Kreatorstore - Storage] Banner upload attempt ${attempt} error:`, error.message || error);
        ...
        console.warn(`[Kreatorstore - Storage] Exception on banner upload attempt ${attempt}:`, err.message || err);
        ...
        console.warn('[Kreatorstore - Storage] Falling back to compressed Base64 data URL for banner.');
        const base64Url = await compressImage(file, 800, 500);
        console.log("Upload Success:", base64Url);
        return base64Url;
      }
