    ...
    try {
      ...
      if (error) {
        if (error.message?.includes('Bucket not found') || error.message?.includes('bucket_not_found')) {
          console.warn('[Kreatorstore - Storage] Bucket "profile-photos" not found. Creating it...');
          try {
  ...
  uploadVerificationDocument: async (file, documentType, creatorId) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ [Kreatorstore - Storage] Storage upload failed, falling back to database base64 storage:', storageErr.message);
        const reader = new FileReader();
        const p = new Promise((resolve) => {
          reader.onloadend = () => resolve(reader.result);
        });
